...
import { SVGRenderer, VNodePatcher, PresenceOverlay, ViewportController } from '@grafloria/renderer';
...
export function runPerfSuite(container: HTMLElement, counts: number[]): PerfSample[] {
  for (const count of counts) {
    ...
    const overlay = new PresenceOverlay({
      ...
      //
      // The overlay's interpolation runs on rAF. This benchmark loop is synchronous, so with
      // the real rAF the cursor callbacks would be QUEUED and never run — every DOM write the
      // overlay does would happen after the last measurement, and `idle-frame-presence` would
      // report 0.0ms because the overlay had done NOTHING. A green number measuring an
      // overlay that never drew anything is worse than no number at all.
      //
      ...
      // means "the overlay did all of its real work and the diagram still paid nothing",
      // which is the claim.
      requestFrame: (cb) => {
        cb();
        return 0;
    ...
    for (let f = 0; f < 20; f++) {
      ...
      overlayFrames.push(now() - tOverlay);
    ...
    // WHAT PRESENCE ITSELF COSTS. Reported so the 0.0ms above cannot be read as "the overlay
    ...
    record({
      ...
      nodes: count,
      links: linkCount,
      ms: median(overlayFrames),
      worstMs: Math.max(...overlayFrames),
      frames: overlay.framesRun,
    });
