...
// ===========================================================================
// ===========================================================================
// FIXED AT MERGE, in collab/capture.ts, exactly where this report said it belonged: an
// EPHEMERAL set beside the DERIVED set that already existed for the same reason. These
// tests are INVERTED — they were written to characterise the defect, and they now guard
// the fix. That is the right end state for a bug that a real browser found and that every
// unit test in the codebase was blind to.
// ===========================================================================
describe('THE EPHEMERAL-STATE LEAK (fixed — these now guard it)', () => {
  let hub: MemoryHub;
  let left: Pane;
  let right: Pane;
  ...
  it('HOVERING a node writes NOTHING to the document, and does not touch the other user', () => {
    const model = left.instance.getModel();
    ...
    // NOT ONE OP. A user moving a mouse is not editing a document, and an append-only
    // history must not fill with thousands of hover events that outlive the session.
    expect(left.session.replica.history().length).toBe(logBefore);

    // …and Bo's node is untouched, because Ana's cursor is Ana's business.
    expect(right.instance.getModel().getNode('hovered-node')!.state.hovered).toBe(false);
  });

  it('SELECTING a node does not select it for the other user', () => {
    const model = left.instance.getModel();
    model.addNode(node('selectme', 200, 150));
    left.session.flush();

    model.selectNode(model.getNode('selectme')!);
    left.session.flush(); // ← the flush that was missing when this test was a FALSE GREEN

    // Selection is per-VIEWER. Ana's click is Ana's; Bo's document is unmoved. (Bo can still
    // SEE that Ana has it selected — that arrives on the awareness channel, per viewer,
    // expiring on disconnect, which is exactly where it belongs.)
    expect(right.instance.getModel().getNode('selectme')!.state.selected).toBe(false);
    expect(left.instance.getModel().getNode('selectme')!.state.selected).toBe(true);
  });
