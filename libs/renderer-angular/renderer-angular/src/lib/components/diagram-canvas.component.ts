...
export class DiagramCanvasComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input() canvasBounds: Rectangle | null = null;

  /** Card 5: double-click a node to edit its label in place. */
  @Input() enableInPlaceEditing = true;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['canvasBounds']) {
      this.snapController.updateConfig({ keepInBounds: this.canvasBounds ?? null });
    }
  ...
  private initializeWave4Controllers(): void {
    ...
    this.snapController.syncWithEngineConfig(this.engine);
    ...
    this.highlighterController.refreshValidation(this.engine);
  ...
  private updateOverlays(): void {
    ...
    this.toolLayer =
      this.enableSelectionTools && !this.isDraggingNode
        ? this.selectionTools.computeLayer(this.engine, this.zoom)
        : { bounds: null, rotation: 0, center: null, nodeIds: [], linkIds: [], handles: [] };

    this.highlighters = this.highlighterController.compute(this.engine);
    this.focusRing = this.enableKeyboardNavigation
      ? this.keyboardNav.getFocusRing(this.engine)
      : null;
  }
  ...
  private onToolHandleDown(handle: ToolHandle, worldX: number, worldY: number): void {
    ...
    switch (handle.kind) {
      case 'resize':
        this.syncSnapScale();
        this.selectionTools.beginResize(handle, this.engine, worldX, worldY);
        return;

      case 'rotate':
        this.selectionTools.beginRotate(handle, this.engine, worldX, worldY);
        return;
      ...
      case 'vertex-remove':
        ...
        this.selectionTools.beginVertexDrag(handle, this.engine);
        return;

      case 'vertex-add': {
        const command = this.selectionTools.addVertexCommand(handle, this.engine);
        if (command) this.executeCommand(command);
        return;
      }

      case 'remove': {
        const command = this.selectionTools.removeSelectionCommand(this.engine);
        if (command) this.executeCommand(command);
        return;
      }
      ...
      case 'link-endpoint': {
        const link = handle.linkId ? this.engine.getDiagram()?.getLink(handle.linkId) : undefined;
  ...
  private onHaloAction(handle: ToolHandle): void {
    ...
    switch (handle.action) {
      case 'connect': {
        ...
        this.interactionHandler.startConnection(port, at.x, at.y, this.engine);
        this.scheduleRender();
        return;
      }
      ...
      case 'clone': {
        ...
        const command = this.selectionTools.cloneNodeCommand(this.engine, handle.nodeId);
        if (command) this.executeCommand(command);
        return;
      }
      ...
      case 'fork': {
        if (!handle.nodeId) return;
        const command = this.selectionTools.forkNodeCommand(this.engine, handle.nodeId);
        if (command) this.executeCommand(command);
        return;
      }
  ...
  private updateToolGesture(event: MouseEvent): void {
    ...
    switch (this.selectionTools.activeGesture()) {
      case 'resize':
        changed = this.selectionTools.updateResize(this.engine, worldX, worldY, modifiers, (box) =>
          this.snapBox(box, this.toolLayer.nodeIds)
        );
        break;
      case 'rotate':
        changed = this.selectionTools.updateRotate(this.engine, worldX, worldY, modifiers);
        break;
      case 'vertex':
        changed = this.selectionTools.updateVertexDrag(this.engine, worldX, worldY, (p) =>
  ...
  private endToolGesture(): void {
    const command = this.selectionTools.endGesture(this.engine);
    if (command) {
      this.executeCommand(command);
    } else if (this.pendingVertexHandle) {
      const remove = this.selectionTools.removeVertexCommand(this.pendingVertexHandle, this.engine);
  ...
  private snapBox(box: Rectangle, excludeIds: string[]): Rectangle {
    ...
    const result = this.snapController.computeSnap(
      box,
      this.snapController.siblingBoxes(this.engine, excludeIds)
    );
  ...
  private clearGuides(): void {
    ...
    if (this.proximityCandidate) {
      this.snapController.highlightProximityTarget(this.engine, null);
  ...
  private updateNodeDrag(current: ToolPointerEvent, down: ToolPointerEvent): void {
    ...
    if (this.enableProximityConnect && primaryId) {
      ...
      this.snapController.highlightProximityTarget(this.engine, this.proximityCandidate);
    }
  ...
  private endMarquee(_current: ToolPointerEvent): void {
    ...
    if (this.marquee) {
      ...
      if (this.enableKeyboardNavigation) {
        this.keyboardNav.announceSelection(this.engine);
      }
    }
    this.marquee = null;
  ...
  onMouseLeave(): void {
    ...
    if (this.selectionTools.isActive()) {
      this.selectionTools.cancelGesture(this.engine);
      this.pendingVertexHandle = null;
    }
    this.clearGuides();
  ...
  private openLinkLabelEditor(
    ...
    const session = this.inPlaceEditor.begin(this.engine, {
      type: 'link-label',
      linkId: link.id,
      labelIndex,
    });
  ...
  private openNodeLabelEditor(nodeId: string): void {
    const session = this.inPlaceEditor.begin(this.engine, { type: 'node', nodeId });
    if (session) {
      this.openTextEditor(session.value, session.center, session.multiline);
    }
  }
  ...
  private openTextEditor(value: string, worldCenter: Point, multiline: boolean): void {
    ...
    const commit = () => {
      if (settled) return;
      settled = true;
      const command = this.inPlaceEditor.commit(this.engine, input.value);
      cleanup();
      if (command) {
        this.executeCommand(command);
      }
  ...
  private handleKeyboardNavigation(event: KeyboardEvent): boolean {
    ...
    if (key === 'Tab') {
      ...
      if (this.keyboardNav.isConnecting()) {
        this.keyboardNav.cycleTargetNode(this.engine, event.shiftKey ? -1 : 1);
      } else if (event.shiftKey) {
        this.keyboardNav.focusPrevious(this.engine);
      } else {
        this.keyboardNav.focusNext(this.engine);
      }
      this.scheduleRender();
      this.cdr.markForCheck();
      return true;
    ...
    if (key === 'Enter') {
      if (this.keyboardNav.isConnecting()) {
        event.preventDefault();
        const command = this.keyboardNav.confirmConnect(this.engine);
        if (command) {
          this.executeCommand(command);
        }
        this.cdr.markForCheck();
      ...
      if (focused) {
        event.preventDefault();
        this.keyboardNav.selectFocused(this.engine);
    ...
    if ((key === 'c' || key === 'C') && !this.keyboardNav.isConnecting()) {
      ...
      event.preventDefault();
      this.keyboardNav.beginConnect(this.engine);
      this.cdr.markForCheck();
      return true;
    }
    ...
    if (diagram.getSelectedNodes().length > 0) {
      const command = this.keyboardNav.nudgeCommand(this.engine, nudge.x, nudge.y);
      if (command) {
        this.executeCommand(command);
      }
      return true;
    ...
    const direction =
      key === 'ArrowLeft'
        ...
        ? 'up'
        : 'down';
    this.keyboardNav.focusDirection(this.engine, direction);
    this.scheduleRender();
    this.cdr.markForCheck();
    return true;
  }
