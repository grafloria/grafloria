...
import { GrafloriaFlow, LIGHT_THEME } from '@grafloria/vue';
import type { DiagramInstance } from '@grafloria/vue';
import { shadcnBridge, muiBridge, tailwindBridge } from '@grafloria/element';
import type { TokenBridge } from '@grafloria/element';
...
// The host's own design-token variables (shadcn / MUI / Tailwind), scoped to
// the wrapper so getComputedStyle resolves them for the bridge.
...
const hostVars = computed(() =>
  hostDark.value ? { ...LIGHT_VARS, ...DARK_VARS } : LIGHT_VARS
);

const map: Record<string, TokenBridge | undefined> = {
...
function use(which: string) {
  active.value = which;
  apiRef?.setTokenBridge(map[which] ?? null);
  apiRef?.renderNow();
}
function flipHost() { hostDark.value = !hostDark.value; apiRef?.renderNow(); }
</script>

<template>
  <div :style="[{ display:'flex', flexDirection:'column', height:'100vh' }, hostVars]">
    <div style="display:flex; gap:8px; align-items:center; padding:10px 24px; border-bottom:1px solid rgba(127,127,127,.25); flex-wrap:wrap">
      ...
      <button v-for="b in bridges" :key="b.key" @click="use(b.key)" :aria-pressed="b.key === active"
        :style="{ padding:'6px 14px', borderRadius:'6px', border:'1px solid rgba(127,127,127,.4)', cursor:'pointer',
                  background: b.key === active ? '#2563eb' : 'transparent',
                  color: b.key === active ? '#fff' : 'inherit',
                  borderColor: b.key === active ? '#2563eb' : 'rgba(127,127,127,.4)' }">{{ b.label }}</button>
      <button @click="flipHost" style="margin-left:16px; padding:6px 14px; border-radius:6px; border:1px solid rgba(127,127,127,.4); background:transparent; color:inherit; cursor:pointer">flip the HOST palette (.dark)</button>
    </div>
    <div style="flex:1">
      <GrafloriaFlow :default-nodes="nodes" :default-edges="edges" :theme="theme" @init="onInit" style="display:block; height:100%" />
    </div>
  </div>
</template>
