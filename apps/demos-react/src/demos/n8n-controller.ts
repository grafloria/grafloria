// The n8n-workflow executor + NDV controller. This holds ALL the imperative
// engine machinery (topological executor, per-node statuses, "n items" wire
// pills, AI sub-node flash, If-branching, pause/step/continue, execution log,
// palette, Node Details View) — translated VERBATIM in behaviour from the
// Angular twin (n8n-workflow.component.ts). The React component owns only the
// JSX; it reads this controller's public fields and calls bump() to re-render.
...
export interface LogRow { title: string; sub: string; status: string; ms: number; nIn: number; nOut: number; note: string; }
export interface Field { type: string; key?: string; label?: string; options?: string[]; code?: boolean; text?: string; }
export interface Pane { items?: any[]; count: string; hint?: string; prevBtn?: boolean }
export interface PalItem { kind: string; glyph: string; sub: string; mono: boolean; bg: string }
...
export class N8nController {
  ...
  // registry (mutable, editable NDV state)
  ...
  // ---- public UI state (React reads these) --------------------------------
  hudText = '';
  readout = 'idle';
  logsOpen = false;
  logRows: LogRow[] = [];
  ...
  ndvFields: Field[] = [];
  ndvInput: Pane = { count: '' };
  ndvOutput: Pane = { count: '' };

  palItems: PalItem[] = ['http', 'set', 'code', 'if', 'merge', 'ai'].map((kind) => ({
  ...
  /** Triggers a React re-render; set by the component. */
  bump: () => void = () => {};

  init(instance: any): boolean {
    this.api = instance;
    this.engine = instance.getEngine?.();
    this.model = instance.getModel?.() ?? this.engine?.getDiagram?.();
  ...
  private updateHud() { this.hudText = `${this.model.getNodes().length} nodes · ${this.model.getLinks().length} wires`; this.bump(); }

  // ---- the execution log --------------------------------------------------
  private setLogsOpen(on: boolean) { this.logsOpen = on; this.bump(); }
  private setLogStatus(text: string, cls = '') { this.logStatusText = text; this.logStatusCls = cls; this.bump(); }
  ...
  onToggleFail() { this.failArmed = !this.failArmed; this.bump(); }
  ...
  onSetFieldName(i: number, v: string) { this.setFields[i].name = v; this.bump(); }
  onSetFieldValue(i: number, v: string) { this.setFields[i].value = v; this.bump(); }
  addSetField() {
  ...
  onSettingToggle(key: string, checked: boolean) { if (this.ndvOpenId) this.SETTINGS[this.ndvOpenId][key] = checked; this.bump(); }
  onNotesInput(v: string) { if (this.ndvOpenId) this.SETTINGS[this.ndvOpenId].notes = v; this.bump(); }

  private renderPaneView(which: 'input' | 'output'): Pane {
  ...
  closeNDV() { this.ndvOpenId = null; this.ndvOpen = false; this.bump(); }
  onNdvName(v: string) {
    if (!this.ndvOpenId) return;
    this.ndvName = v;
  ...
  setTab(t: 'params' | 'settings') { this.ndvTab = t; this.bump(); }
  ...
  onDblClick(clientX: number, clientY: number, host: HTMLElement) {
  ...
  // Static spec data for the component's initial mount.
  static spec() { return buildSpec(); }
}
