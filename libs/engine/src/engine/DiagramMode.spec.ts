...
describe('Diagram Mode System (Phase 1.5)', () => {
  ...
  // wave14/model — the five 'Mode-Aware Behavior' describes DELETED: they exercised
  // DiagramEngine.getNodeBehaviorForMode, a method with no caller outside this spec
  // (the real mode enforcement is the wave-9 ReadonlyLock, covered by
  // models/readonly-lock.spec.ts). Mode management/transition/persistence coverage
  // below is untouched.
  ...
  // wave14/model — 'Priority 2b: Per-Entity Behavior Overrides' DELETED with the
  // NodeModel.behaviorOverrides chain it exercised (dead outside this spec; the real
  // read-only mechanism is the wave-9 ReadonlyLock). Legacy-document tolerance for the
  // serialized key is asserted in models/NodeModel.legacy-keys.spec.ts.
