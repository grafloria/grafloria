...
export class ASTTransformer {
    ...
    transform(ast, options = {}) {
        // Apply options
        const { defaultNodeSize = { width: 120, height: 80 }, nodeSpacing = 150, startPosition = { x: 100, y: 100 }, autoPosition = true, diagramName = 'Generated Diagram', } = options;
        this.nodeSpacing = nodeSpacing;
        this.nextAutoPosition = Object.assign({}, startPosition);
        // Create diagram
     ...
     */
    processStatement(statement, diagram, defaultNodeSize, autoPosition) {
        switch (statement.type) {
            case 'NodeDefinition':
                this.createNode(statement, diagram, defaultNodeSize, autoPosition);
                break;
            case 'EdgeDefinition':
                // Edges are processed after all nodes are created
                break;
            case 'Subgraph':
                this.createSubgraph(statement, diagram, defaultNodeSize, autoPosition);
                break;
            case 'ClassDef':
    ...
    flattenStatements(statements) {
        ...
        for (const st of statements) {
            out.push(st);
            if (st.type === 'Subgraph')
                out.push(...this.flattenStatements(st.statements));
        }
        return out;
    }
    /**
    ...
    applyDirectives(statements, diagram) {
        for (const st of statements) {
            if (st.type === 'Style')
                this.applyStyle(st, diagram);
            else if (st.type === 'ClassApplication')
                this.applyClass(st, diagram);
            else if (st.type === 'Click')
                this.applyClick(st, diagram);
            else if (st.type === 'GrafloriaDirective' && st.target === 'node') {
                this.applyGrafloriaNode(st, diagram);
            }
        }
        for (const st of statements) {
            if (st.type === 'LinkStyle')
                this.applyLinkStyle(st, diagram);
    ...
    applyClass(node, diagram) {
        ...
        for (const id of node.ids) {
            ...
            if (target)
                this.applyStyleToNode(target, props);
        }
    }
    ...
    applyClick(node, diagram) {
        const target = diagram.getNode(node.id);
        if (!target)
            return;
        if (node.href)
            target.setMetadata('href', node.href);
        if (node.tooltip)
            target.setMetadata('tooltip', node.tooltip);
    }
    /** `%%grafloria:node a status:running` — Grafloria-only node state. */
    applyGrafloriaNode(node, diagram) {
    ...
    applyGrafloriaEdge(node, diagram) {
        ...
        if (node.properties['animation']) {
            ...
            if (node.properties['speed'])
                anim['speed'] = node.properties['speed'];
     ...
     */
    createNode(astNode, diagram, defaultNodeSize, autoPosition) {
        ...
        if (astNode.style) {
            ...
            const updatedShapeConfig = Object.assign({}, shapeConfig);
            if (astNode.style.fill)
                updatedShapeConfig.fill = astNode.style.fill;
            if (astNode.style.stroke)
                updatedShapeConfig.stroke = astNode.style.stroke;
            if (astNode.style.strokeWidth !== undefined)
                updatedShapeConfig.strokeWidth = astNode.style.strokeWidth;
    ...
    createLink(astEdge, diagram) {
        ...
        if (!sourceNode) {
            ...
            }, diagram, { width: 120, height: 80 }, true);
        }
     ...
     */
    createSubgraph(astSubgraph, diagram, defaultNodeSize, autoPosition) {
        ...
        for (const id of memberIds) {
            if (diagram.getNode(id))
                group.addMember(id, diagram);
        }
     ...
     */
    collectDirectNodeIds(statements, into) {
        for (const statement of statements) {
            if (statement.type === 'NodeDefinition') {
                into.add(statement.id);
            }
            else if (statement.type === 'EdgeDefinition') {
                into.add(statement.source);
                into.add(statement.target);
            }
        }
    }
    /**
     ...
     */
    storeClassDef(classDefNode, diagram) {
        const classDefs = diagram.getMetadata('classDefs') || {};
     ...
     */
    applyStyleToNode(node, properties) {
        if (properties.fill) {
        ...
        const shapeConfig = Object.assign({}, node.getMetadata('shape'));
        if (properties.fill)
            shapeConfig['fill'] = properties.fill;
        if (properties.stroke)
            shapeConfig['stroke'] = properties.stroke;
        if (properties.strokeWidth !== undefined)
            shapeConfig['strokeWidth'] = properties.strokeWidth;
        node.setMetadata('shape', shapeConfig);
     ...
     */
    applyLinkTypeStyle(link, linkType) {
        switch (linkType) {
            case 'dotted-line':
            case 'dotted-arrow':
     ...
     * Get size for shape
     */
    getSizeForShape(shape, defaultSize) {
     ...
     */
    getPathTypeFromLinkType(linkType) {
     ...
     */
    getNextAutoPosition() {
        const position = Object.assign({}, this.nextAutoPosition);
