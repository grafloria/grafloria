...
import { DSLGenerator } from './generator/DSLGenerator';
import { DSLFormatter } from './generator/DSLFormatter';
...
import { parseMermaidEr, erModelToDiagram, generateErFromDiagram, parseMermaidClass, classModelToDiagram, generateClassFromDiagram, parseMermaidState, stateModelToDiagram, generateStateFromDiagram, } from './mermaid';
...
export class DSL {
    constructor(options = {}) {
        ...
        this.options = Object.assign({ autoLayout: true, debug: false }, options);
    }
    /**
    ...
    parseDetailed(text) {
        ...
        try {
            ...
            if (diagramType === 'classDiagram') {
                return this.finishGraphType(classModelToDiagram(parseMermaidClass(text)), 'classDiagram', startTime);
            }
            if (diagramType === 'stateDiagram' || diagramType === 'stateDiagram-v2') {
                return this.finishGraphType(stateModelToDiagram(parseMermaidState(text)), 'stateDiagram-v2', startTime);
            }
            ...
            if (this.options.autoLayout) {
                ...
                if (this.options.debug) {
                    console.log(`[DSL] Layout suggestion: ${layoutSuggestion.presetId} (confidence: ${layoutSuggestion.confidence.toFixed(2)})`);
            ...
            if (error instanceof ParseError) {
                throw new Error(`DSL Parse Error at line ${error.line}, column ${error.column}: ${error.message}`);
            }
            throw error;
        }
    }
    ...
    validate(text) {
        ...
        try {
            ...
            if (error instanceof ParseError) {
                errors.push(`Line ${error.line}, Column ${error.column}: ${error.message}`);
            }
    ...
    generate(diagram, options) {
        ...
        if (graphType === 'erDiagram' || graphType === 'erd')
     ...
     */
    generateFormatted(diagram, generatorOptions, formatterOptions) {
     ...
     */
    generatePretty(diagram, addHeaders = true) {
    ...
    testRoundTrip(text) {
        try {
            ...
            const success = originalNodes === regeneratedNodes &&
    ...
    parseERD(text) {
        ...
        if (this.options.debug) {
            console.log(`[DSL] Parsed ERD: ${model.entities.length} entities, ${model.relationships.length} relationships`);
        }
    ...
    parseUML(text) {
        ...
        if (this.options.debug) {
            console.log(`[DSL] Parsed UML: ${model.classes.length} classes, ${model.relationships.length} relationships`);
        }
     ...
     */
    applyStylesAndTemplates(diagram, text, styleDefinitions, templateDefinitions) {
        ...
        for (const node of diagram.getNodes()) {
            ...
            const templateName = node.data['template'];
     ...
     */
    finishGraphType(diagram, astType, startTime) {
        return {
            diagram,
            ast: { type: 'Diagram', diagramType: astType, direction: 'TD', statements: [] },
            tokens: [],
            stats: {
