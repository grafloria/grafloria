...
expectThat(
  'KNOWN BUG (capture.ts, wave9/crdt): merely HOVERING a node writes ops into the shared document',
  bAfterHover.logLength > logBeforeHover,
  `log grew ${logBeforeHover} → ${bAfterHover.logLength} from a mouse passing over a node — ` +
    `state.hovered is captured as a document edit. Reported, not fixed (collab/ is not this card).`
);
