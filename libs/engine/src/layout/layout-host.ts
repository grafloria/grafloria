...
function defaultResolver(): (name: string) => LayoutAdapter | undefined {
  ...
  return (name) => {
    ...
    // MERGE (Cards 2 + 3): every algorithm the host runs is wrapped in COMPONENT
    // PACKING before it is handed anything.
    //
    // Card 2 made packing a property of every registered layout by wrapping
    // `apply(diagram)`. Card 3 then routed the flat path through this host, which
    // ships `(nodes, links)` and runs the RAW adapter — stepping neatly around the
    // wrapper. A disconnected graph promptly went back to laying its components on
    // top of each other, which is precisely the bug Card 2 exists to kill (and its
    // forest tests caught it the moment the two branches met).
    //
    // Wrapping here rather than at the call site means the INLINE and the WORKER
    // paths pack identically — the same one-protocol discipline the rest of this
    // file is built on. Packing is a provable no-op for a connected graph, so this
    // costs nothing when there is nothing to pack.
    const raw = make();
    ...
    // `isSteppable()` goes false, and force silently drops off the steppable path
    // onto the opaque one: no progress, no cancellation, no partial result. (That
    // is exactly what happened when these two branches first met, and Card 3's own
    // tests caught it.) Delegating through the prototype keeps the algorithm whole
    // and overrides only `apply`.
    const adapter: LayoutAdapter = Object.create(raw);
    adapter.apply = (nodes, links, options) =>
      layoutWithComponentPacking(
        raw.name,
        (n, l, o) => raw.apply(n, l, o),
        nodes,
        links,
...
export function serveLayout(port: LayoutServePort, deps: ServeLayoutDeps = {}): void {
  ...
  async function run(request: LayoutRequestRun): Promise<void> {
    ...
    try {
      ...
      //
      // TWO CARDS MEET HERE, AND ONE HAS TO YIELD. A steppable algorithm is driven
      // through step()/snapshot(), which BYPASSES apply() — and apply() is where
      // Card 2's component packing lives. So a disconnected graph run on the
      // steppable path comes back with its components laid on top of each other:
      // exactly the bug packing exists to kill.
      //
      // A forest therefore takes the packed path even when the algorithm could have
      // been interrupted. The trade is stated rather than hidden: you lose mid-run
      // cancellation on a DISCONNECTED force layout (it still runs off the main
      // thread, and can still be cancelled before it starts), and you keep a picture
      // that is actually readable. A connected graph — the overwhelmingly common
      // case, and the one where a long force run actually hurts — is unaffected.
      const componentCount = findConnectedComponents(nodes, links).length;
      if (isSteppable(adapter) && componentCount <= 1) {
