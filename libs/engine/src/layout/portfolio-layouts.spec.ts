...
describe('Card 2 — the portfolio is reachable through engine.layout()', () => {
  ...
  it('an unknown layout still fails loudly, and now lists the portfolio', async () => {
    ...
    // The list now also carries `layered` (Cards 1 & 5) and `auto` (Card 7b), which
    // landed in parallel. Assert the CONTRACT — the error names what is available —
    // rather than freezing a list that every future card has to come back and edit.
    await expect(engine.layout('nope')).rejects.toThrow(/Unknown layout 'nope'/);
    await expect(engine.layout('nope')).rejects.toThrow(/circular/);
    await expect(engine.layout('nope')).rejects.toThrow(/tree/);
    engine.destroy();
  });
