...
const STATE_DECL_RE = new RegExp(`^state\\s+(${ID}${CSS})\\s*(?:<<(fork|join|choice)>>)?\\s*(\\{?)\\s*$`);
...
function hasConcurrentRegions(lines, open) {
    let depth = 0;
    for (let i = open + 1; i < lines.length; i++) {
        const text = lines[i].text;
        if (text === '}') {
            if (depth === 0)
                return false; // end of the composite we asked about
            depth--;
            continue;
        }
        if (depth === 0 && REGION_SEPARATOR_RE.test(text))
            return true;
        const decl = text.match(STATE_DECL_RE);
        const as = text.match(STATE_AS_RE);
        if ((decl === null || decl === void 0 ? void 0 : decl[3]) === '{' || (as === null || as === void 0 ? void 0 : as[3]) === '{')
            depth++;
    }
    return false;
}
 ...
 */
export function parseMermaidState(text) {
    var _a;
    ...
    const notes = [];
    let direction;
    const touch = (id, scope, kind = 'state') => {
        const existing = states.get(id);
        if (existing) {
            if (kind !== 'state' && existing.kind === 'state')
                existing.kind = kind;
            return existing;
        }
        const node = Object.assign({ id, label: id, kind }, (scope ? { parent: scope } : {}));
        states.set(id, node);
        return node;
    };
    /**
    ...
    const scope = () => {
        var _a;
        const f = frame();
        return f ? ((_a = f.region) !== null && _a !== void 0 ? _a : f.id) : undefined;
    };
    ...
    const openComposite = (node, at) => {
        node.composite = true;
        const f = { id: node.id, concurrent: hasConcurrentRegions(lines, at), regions: 0 };
        frames.push(f);
        if (f.concurrent) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (REGION_SEPARATOR_RE.test(line)) {
            ...
            const f = frame();
            if (f === null || f === void 0 ? void 0 : f.concurrent)
                openRegion(f);
            continue;
        }
        ...
        if (dir) {
            ...
            if (owner)
                owner.direction = dir[1].toUpperCase();
            else
        ...
        if (transition) {
            ...
            transitions.push(Object.assign({ from, to }, (label && label.trim() ? { label: unquote(label.trim()) } : {})));
            continue;
        }
        ...
        if (stateAs) {
            ...
            if (stateAs[3] === '{')
                openComposite(node, i);
            continue;
        }
        const decl = line.match(STATE_DECL_RE);
        if (decl) {
            const [, id, pseudoKind, brace] = decl;
            const node = touchStyled(id, (_a = pseudoKind) !== null && _a !== void 0 ? _a : 'state');
            if (brace === '{')
                openComposite(node, i);
            continue;
        }
    ...
    return Object.assign({ states: [...states.values()], transitions, notes }, (direction ? { direction } : {}));
}
// ── Model building ─────────────────────────────────────────────────────────
const PSEUDO_SIZE = 22;
export function stateModelToDiagram(model) {
    ...
    if (model.notes.length)
        diagram.setMetadata('stateNotes', model.notes);
    const nodes = new Map();
    model.states.forEach((state, i) => {
        ...
        const shape = isPseudo ? 'circle' : state.kind === 'choice' ? 'rhombus' : 'rounded-rectangle';
        node.setMetadata('dslShape', shape);
        node.setMetadata('shape', Object.assign({ type: isPseudo ? 'circle' : state.kind === 'choice' ? 'diamond' : 'rect', cornerRadius: isPseudo || state.kind === 'choice' ? 0 : 8 }, (state.kind === 'start' ? { fill: '#0f172a' } : {})));
    ...
    for (const state of model.states) {
        ...
        diagram.addGroup(new GroupModel({ id: `group-${state.id}`, name: state.region ? '' : state.label || state.id }));
    }
    for (const state of model.states) {
        if (!state.composite)
            continue;
        ...
        for (const child of model.states) {
            ...
            if (diagram.getNode(child.id))
                group.addMember(child.id, diagram);
        }
    }
    for (const transition of model.transitions) {
        ...
        if (!link)
            continue;
        if (transition.label)
            link.setLabel(transition.label);
        link.setMetadata('stateTransition', transition);
    }
    return diagram;
}
export function stateModelFromDiagram(diagram) {
    var _a, _b, _c, _d, _e;
    const states = [];
    for (const node of diagram.getNodes()) {
        const stored = node.getMetadata('stateNode');
        states.push(stored
            ? Object.assign(Object.assign({}, stored), { id: node.id, label: (_b = (_a = node.getLabel) === null || _a === void 0 ? void 0 : _a.call(node)) !== null && _b !== void 0 ? _b : stored.label }) : { id: node.id, label: (_d = (_c = node.getLabel) === null || _c === void 0 ? void 0 : _c.call(node)) !== null && _d !== void 0 ? _d : node.id, kind: 'state' });
    }
    const transitions = [];
    for (const link of diagram.getLinks()) {
        ...
        transitions.push(Object.assign({ from, to }, (label ? { label } : {})));
    }
    const direction = diagram.getMetadata('direction');
    const notes = (_e = diagram.getMetadata('stateNotes')) !== null && _e !== void 0 ? _e : [];
    return Object.assign({ states, transitions, notes }, (direction ? { direction } : {}));
}
// ── Generation ─────────────────────────────────────────────────────────────
/**
...
export function generateMermaidState(model) {
    ...
    const children = (parent) => model.states.filter((s) => { var _a; return ((_a = s.parent) !== null && _a !== void 0 ? _a : undefined) === parent; });
    ...
    const emitTransitions = (owner, indent) => {
        var _a, _b;
        for (const t of model.transitions) {
            if (emitted.has(t))
                continue;
            if (((_a = byId.get(t.from)) === null || _a === void 0 ? void 0 : _a.parent) !== owner || ((_b = byId.get(t.to)) === null || _b === void 0 ? void 0 : _b.parent) !== owner)
                continue;
            emitted.add(t);
    ...
    const emitBody = (owner, indent) => {
        for (const child of children(owner.id))
            emitState(child, indent);
        emitTransitions(owner.id, indent);
    };
    const emitState = (state, indent) => {
        if (PSEUDO_KINDS.has(state.kind))
            return; // written as [*] / [H] / [H*]
        if (state.kind !== 'state') {
            lines.push(`${indent}state ${state.id} <<${state.kind}>>`);
            return;
        }
        if (state.composite) {
            const head = state.label && state.label !== state.id
                ? `state "${state.label}" as ${state.id}`
                : `state ${state.id}`;
            lines.push(`${indent}${head} {`);
            if (state.direction)
                lines.push(`${indent}    direction ${state.direction}`);
            if (state.concurrent) {
    ...
    for (const state of children(undefined))
        emitState(state, '    ');
    for (const t of model.transitions) {
        if (emitted.has(t))
            continue;
...
export function generateStateFromDiagram(diagram) {
    return generateMermaidState(stateModelFromDiagram(diagram));
}
//# sourceMappingURL=MermaidState.js.map